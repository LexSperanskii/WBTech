...
class InteractorGetListOfEventsTest {

    private lateinit var useCase: EventsUseCase
    private lateinit var networkRepository: INetworkRepository
    private lateinit var systemUnderTest: InteractorGetListOfEventsImpl
    ...
    @OptIn(ExperimentalCoroutinesApi::class)
    @Test
    fun `invoke should return list of events from network repository`() = runTest {

        whenever(useCase.observeListOfEvents()).thenReturn(flowOf(Unit))
        whenever(networkRepository.getListOfEvents()).thenReturn(flowOf(stubData))

        systemUnderTest = InteractorGetListOfEventsImpl(useCase, networkRepository)

        val result = systemUnderTest.invoke().first()

        verify(useCase).observeListOfEvents()
        verify(networkRepository).getListOfEvents()
